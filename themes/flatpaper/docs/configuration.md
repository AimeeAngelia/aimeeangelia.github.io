...
Menu icons can also be images: use an `http(s)` URL or a local path ending in an image extension (`.png`, `.jpg`, `.jpeg`, `.webp`, `.gif`, `.svg`, `.ico`). The image is rendered inline at the same size as the icon glyph:

```yaml
menu:
  Travellings:
    link: https://www.travellings.cn/go.html
    icon: /images/travellings.png
```
